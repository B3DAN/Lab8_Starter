describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    await button.click();
    let innerHTML = await button.getProperty('innerText');
    let buttonText = await innerHTML.jsonValue();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    let cartCountNum = await cartCount.getProperty('innerText');
    let Num = await cartCountNum.jsonValue();
    expect(Num).toBe("20");
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 0; i < prodItems.length; i++){
      ...
      if(buttonText != "Remove from Cart"){
        stillRemoveFromCart = false;
      }
    }

    expect(stillRemoveFromCart).toBe(true);
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    let localCall = await page.evaluate(() =>  {return window.localStorage.getItem('cart')});
    //let currCart = await localCall.jsonValue();
    let currArray = await JSON.parse(localCall);
    let allmatch = true;
